...
public func fittedRect(aspect: CGFloat, in size: CGSize) -> CGRect {
...
public func fittedSize(aspect: CGFloat, in size: CGSize) -> CGSize {
...
public func clamp(_ v: CGFloat, _ lo: CGFloat = 0, _ hi: CGFloat = 1) -> CGFloat {
    min(max(v, lo), hi)
}

/// "HH:MM:SS" elapsed-time string.
public func elapsedString(_ t: TimeInterval) -> String {
    let s = Int(max(0, t))
...
public func clockString(_ date: Date = Date()) -> String { clockFormatter.string(from: date) }
